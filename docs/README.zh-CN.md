...
| `split_tunnel_uid` | 基于 UID 的 split-tunneling 规则（FRA_UID_RANGE） | Netlink `RTM_GETRULE` dump |
...
| `backpressure` | 50000 个 UDP 包下的吞吐量 | `sendto()` 突发 + 计时 |
| `gso_failed` / `gso_send_failed` / `gso_ok` | GSO 大发送（UDP_SEGMENT）— 虚拟接口不支持 | `setsockopt(UDP_SEGMENT)` + `sendto()` |
...
高置信度（→ `detected = true`）：`sysfs_vpn_leak`、`getifaddrs_vpn`、`sysclassnet_vpn`、`rtm_getlink_vpn`、`proc_if_inet6_vpn`、`proc_ipv6_route_vpn`、`proc_net_dev_vpn`、`ifindexname_vpn`、`vpn_policy_rules_netlink`、`split_tunnel_uid`、`bindtodevice_leak`、`getsockname_leak`、`udp_port_conflict_physical`、`gso_failed`、`gso_send_failed`。其余 → `needsReview`。

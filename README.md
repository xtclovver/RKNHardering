...
В таблице маршрутов (`NETLINK`) ищется полностью описанный `unicast`-маршрут со scope `global` или `link`, префиксом `/32` (IPv4) или `/128` (IPv6) и публичным маршрутизируемым адресом через обычный физический интерфейс. Маршрут должен быть создан не ядром и находиться не в таблице `local`; неполные строки не считаются доказательством. Запись может быть обходным маршрутом VPN-клиента к серверу, но по одному FIB её нельзя надёжно отличить от системного или операторского маршрута, особенно на `rmnet*`, `ccmni*`, `pdp*` и `seth*`. Поэтому результат только требует проверки (`EvidenceSource.NATIVE_HOST_ROUTE`) и сам по себе не подтверждает VPN.
